...
class TestNewUserJourney:
    ...
    def test_e2e_006_workflows_list_no_server(self):
        ...
        assert result.returncode != 0
        assert "error" in result.stdout.lower() or "error" in result.stderr.lower() or "connection" in result.stderr.lower()
    ...
    def test_e2e_007_tools_list_no_server(self):
        ...
        if result.returncode == 0:
            assert "0" in result.stdout or "empty" in result.stdout.lower() or "no" in result.stdout.lower()
...
'''
            ...
            'steps': [
                {
                    'id': 'use_tool',
                    'tool': 'echo',
                    'inputs': {'message': 'Hello from tool'}
                }
            ],
    ...
    def test_e2e_013_json_output_format(self):
        ...
        result = subprocess.run(
            ["python", "-m", "ploston_cli", "--json", "version"],
            capture_output=True,
            text=True
        )
    ...
    def test_e2e_014_verbose_mode(self):
        ...
        result = subprocess.run(
            ["python", "-m", "ploston_cli", "-v", "version"],
            capture_output=True,
            text=True
        )
    ...
    def test_e2e_015_quiet_mode(self):
        ...
        result = subprocess.run(
            ["python", "-m", "ploston_cli", "-q", "version"],
            capture_output=True,
            text=True
        )
...
'''
    ...
    def test_e2e_025_workflow_with_math_operations(self):
        ...
        workflow = {
            ...
            'version': '1.0',
            'inputs': [
                {'numbers': {'type': 'string', 'default': '1,2,3,4,5'}}
            ],
            'steps': [
                {
                    'id': 'calculate',
