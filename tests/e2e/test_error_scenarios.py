...
class TestValidationErrors:
    ...
    def test_err_002_missing_version(self):
        ...
        combined = result.stdout + result.stderr
        assert result.returncode != 0 or "error" in combined.lower() or "version" in combined.lower()
    ...
    def test_err_005_duplicate_step_ids(self):
        ...
        combined = result.stdout + result.stderr
        assert result.returncode != 0 or "error" in combined.lower() or "duplicate" in combined.lower()
...
class TestFileErrors:
    ...
    def test_err_010_nonexistent_file(self):
        ...
        assert "error" in combined.lower() or "not found" in combined.lower() or "no such" in combined.lower()
...
class TestConnectionErrors:
    ...
    def test_err_021_connection_refused(self):
        ...
        combined = result.stdout + result.stderr
        assert result.returncode != 0 or "error" in combined.lower() or "connection" in combined.lower()
    ...
    def test_err_022_timeout_handling(self):
        ...
        combined = result.stdout + result.stderr
        assert result.returncode != 0 or "error" in combined.lower() or "timeout" in combined.lower()
    ...
    def test_err_030_missing_required_input(self):
        ...
        result = subprocess.run(
            ["python", "-m", "ploston_cli", "run", str(workflow_file),
             "--server", "http://localhost:19999"],  # Non-existent server
            capture_output=True,
            text=True,
            timeout=10
        )
