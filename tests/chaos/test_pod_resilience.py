...
def kubectl_available():
    ...
    assert result.returncode == 0, \
        "kubectl is not installed. Install kubectl to run chaos tests."
...
def ploston_pods_exist(kubectl_available):
    ...
    assert result.returncode == 0, \
        f"Cannot query ploston pods: {result.stderr}"
...
class TestPodResilience:
    ...
    def test_chaos_001_ploston_pod_restart_recovery(self, kubectl_available, ploston_pods_exist):
        ...
        result = subprocess.run(
            [kubectl_available, "delete", "pod", "-n", NAMESPACE,
             "-l", "app=ploston", "--wait=false"],
            capture_output=True,
            text=True
        )
