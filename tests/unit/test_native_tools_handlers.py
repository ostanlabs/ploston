...
``ploston.native_tools.server`` directly directly (the @mcp.tool decorator now yields plain
callables) and assert on the real output / error envelopes produced by the
...
class TestDataValidate:
    ...
    async def test_valid_data_passes(self):
        ...
        result = await srv.data_validate({"a": 1}, schema)
    ...
    async def test_invalid_data_reports_errors(self):
        schema = {"type": "object", "properties": {"a": {"type": "integer"}}}
        result = await srv.data_validate({"a": "not-an-int"}, schema)
        assert result["success"] is True
...
class TestJsonCsvRoundTrip:
    ...
    async def test_json_to_csv_with_headers(self):
        result = await srv.data_json_to_csv([{"a": 1, "b": 2}, {"a": 3, "b": 4}])
        assert result["success"] is True
    ...
    async def test_csv_to_json_without_headers_uses_generated_columns(self):
        result = await srv.data_csv_to_json("1,2\n3,4", has_headers=False)
        assert result["success"] is True
...
class TestJsonXmlRoundTrip:
    ...
    async def test_json_to_xml(self):
        result = await srv.data_json_to_xml({"a": 1}, root_element="doc")
        assert result["success"] is True
...
class TestExtractText:
    ...
    async def test_extract_from_html_strips_tags(self):
        result = await srv.extract_text("<html><body>Hi <b>there</b></body></html>", "html")
        assert result["success"] is True
    ...
    async def test_extract_from_json(self):
        result = await srv.extract_text('{"a": 1}', "json")
        assert result["success"] is True
    ...
    async def test_extract_auto_plain_text(self):
        result = await srv.extract_text("plain text here", "auto")
        assert result["success"] is True
        assert result["word_count"] == 3

    @pytest.mark.asyncio
    async def test_extract_markdown(self):
        result = await srv.extract_text("# Title\ntext", "markdown")
        assert result["success"] is True
...
class TestExtractStructured:
    ...
    async def test_regex_extraction_finds_matches(self):
        result = await srv.extract_structured("contact a@b.com today", {"email": r"[\w.]+@[\w.]+"})
        assert result["success"] is True
    ...
    async def test_missing_field_reported(self):
        result = await srv.extract_structured("no numbers here", {"digits": r"\d+"})
        assert result["success"] is True
        assert "digits" in result["fields_missing"]

    @pytest.mark.asyncio
    async def test_invalid_regex_returns_error_envelope(self):
        result = await srv.extract_structured("x", {"bad": "("})
        assert result["success"] is False
...
class TestExtractFileMetadata:
    ...
    async def test_metadata_for_existing_file(self, workspace):
        ...
        result = await srv.extract_file_metadata("meta.txt")
        assert result["success"] is True
    ...
    async def test_metadata_missing_file_error(self, workspace):
        result = await srv.extract_file_metadata("nope.txt")
        assert result["success"] is False
...
async def _write(srv_mod, path, content, **kw):
    ...
    return srv_mod.fs_write(path, content, **kw)


class TestFilesystem:
    def test_write_then_read_text(self, workspace):
        wrote = srv.fs_write("sub/a.txt", "hello")
        assert wrote["created"] is True
        assert wrote["size"] == 5

        read = srv.fs_read("sub/a.txt")
        assert read["content"] == "hello"
        assert read["format"] == "text"

    def test_write_then_read_json(self, workspace):
        srv.fs_write("d.json", {"k": 1}, format="json")
        read = srv.fs_read("d.json", format="json")
        assert read["content"] == {"k": 1}

    def test_list_recursive_reports_files_and_dirs(self, workspace):
        srv.fs_write("sub/a.txt", "x")
        srv.fs_write("sub/b.txt", "y")
        listing = srv.fs_list(".", recursive=True)
    ...
    def test_list_with_pattern_filter(self, workspace):
        srv.fs_write("keep.log", "x")
        srv.fs_write("skip.txt", "y")
        listing = srv.fs_list(".", pattern="*.log")
        names = {item["name"] for item in listing["items"]}
    ...
    def test_read_missing_file_raises(self, workspace):
        with pytest.raises(FileNotFoundError):
            srv.fs_read("ghost.txt")

    def test_delete_file(self, workspace):
        srv.fs_write("temp.txt", "x")
        result = srv.fs_delete("temp.txt")
        assert result["deleted"] is True
        with pytest.raises(FileNotFoundError):
            srv.fs_read("temp.txt")

    def test_delete_missing_raises(self, workspace):
        with pytest.raises(FileNotFoundError):
            srv.fs_delete("ghost.txt")

    def test_delete_nonempty_dir_requires_recursive(self, workspace):
        srv.fs_write("d2/x.txt", "x")
        with pytest.raises(ValueError):
            srv.fs_delete("d2")
        result = srv.fs_delete("d2", recursive=True)
        assert result["deleted"] is True
...
class TestNetworkPortCheck:
    ...
    async def test_closed_port_reports_not_open(self):
        ...
        result = await srv.network_port_check("127.0.0.1", 1, timeout=2)
        assert result["success"] is True
    ...
    async def test_invalid_port_rejected(self):
        result = await srv.network_port_check("127.0.0.1", 0)
        assert result["success"] is False
    ...
    async def test_open_port_detected(self):
        ...
        try:
            result = await srv.network_port_check("127.0.0.1", port, timeout=2)
            assert result["success"] is True
...
class TestNetworkDnsLookup:
    ...
    async def test_localhost_resolves(self):
        result = await srv.network_dns_lookup("localhost", "A")
        assert result["success"] is True
        assert any("127.0.0.1" in str(r) for r in result.get("records", []))

    @pytest.mark.asyncio
    async def test_unresolvable_hostname_errors(self):
        result = await srv.network_dns_lookup("definitely-not-a-real-host.invalid", "A")
        assert result["success"] is False
